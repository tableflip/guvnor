...
BossRPC.prototype._restartProcess = function(failedProcess) {
	...
	if(failedProcess.options.restartOnError) {
		...
		if(failedProcess.restarts < failedProcess.options.restartRetries) {
			console.info("Restarting process", failedProcess.script, failedPid, "x", failedProcess.restarts);

			var pid = this.startProcess(failedProcess, function(error) {
				if(error) {
					return console.error("Failed to restart process", failedProcess.script, failedPid, error);
				}
				console.info("Restarted process", failedProcess.script, failedPid, "as", pid);
			});
		}
	}
...
BossRPC.prototype.listProcesses = function(callback) {
	...
	async.parallel(procList.map(function(processInfo) {
		return function(callback) {
			...
			console.info("Requesting status for process", processInfo.process.pid);
