...
LocalDaemonConnection.prototype.connect = function(api, callback) {
  return this._connect(this._config.boss.rundir + '/' + this._socketName, api, callback)
}
...
LocalDaemonConnection.prototype._connect = function(socket, api, callback) {
  ...
  this._client = this._dnode(api, {
    timeout: this._config.boss.rpctimeout
  })
  this._client.connect(socket, function(remote) {
    ...
    for(var method in remote) {
      ...
      if(method == 'deployApplication' ||
        ...
        serverApi[method] = timeoutify(remote[method].bind(remote), this._config.boss.timeout)
      }
    }
  ...
  this._client.on('error', function(error) {
    if (error.code == 'EACCES') {
      return callback(new Error('I don\'t have permission to access ' + socket + ' - please run boss as a user that can.'))
      ...
      return callback(new Error(socket + ' is not a socket - please check the boss.rundir property in your config file'))
    }
