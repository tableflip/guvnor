...
LocalDaemonStarter.prototype._startDaemonWithDebugPort = function(callback, port) {
  ...
  if(port) {
    this._logger.warn('Boss daemon will wait for a debugger connection on port %d before continuing', port)
  ...
  this._daemon = this._child_process.spawn(process.execPath, execArgv, {
    ...
    uid: this._posix.getpwnam(this._config.boss.user).uid,
    gid: this._posix.getgrnam(this._config.boss.group).gid
  })
  this._daemon.on('error', function(event) {
    ...
    if(event.code == 'EPERM') {
      ...
      this._logger.error('Daemon encountered a permissions error - did you start Boss with the right user?')
      process.exit(-1)
    }
  }.bind(this))
  ...
  this._daemon.on('message', function(event) {
    if(event.type == 'boss:config:request') {
      this._logger.debug('Daemon requested config')
      this._daemon.send({type: 'boss:config:response', args: [this._config]})
      ...
      callback = null
    } else if(event.type == 'boss:fatality') {
      ...
      if(error.code == 'EACCES') {
        ...
        error.message = 'Daemon encountered a permissions error - did you start Boss with the right user?'
      }
