...
RemoteDaemon.prototype._connectToDaemon = function (callback) {
  ...
  this._client = this._tls.connect(this._port, {
    ...
    d.on('remote', function (remote) {
      this._logger.info('Connected to daemon on %s:%d', this._host, this._port)
  ...
  this._client.on('end', function () {
    if (!this._disconnecting) {
      this._logger.info('Daemon %s:%d unexpectedly disconnected, will reconnect in a little while', this._host, this._port)
      this._reconnect(callback)
    }
