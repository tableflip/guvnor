...
var ProcessOverviewPage = require('./views/process/overview')
...
module.exports = Router.extend({
  ...
  process: function (hostName, processName) {
    this._withHostAndProcess(hostName, processName, function (host, process) {
      this.trigger('page', new ProcessPage({
        model: process,
        runningPage: ProcessOverviewPage
      }))
  ...
  processLogs: function (hostName, processName) {
    this._withHostAndProcess(hostName, processName, function (host, process) {
      this.trigger('page', new ProcessPage({
        model: process,
        runningPage: ProcessLogsPage
      }))

      App.view.setActiveNav('/host/' + hostName + '/process/' + processName + '/logs')
    }.bind(this))
  },

  processExceptions: function (hostName, processName) {
    this._withHostAndProcess(hostName, processName, function (host, process) {
      this.trigger('page', new ProcessPage({
        model: process,
        runningPage: ProcessExceptionsPage
      }))

      App.view.setActiveNav('/host/' + hostName + '/process/' + processName + '/exceptions')
    }.bind(this))
  },

  processSnapshots: function (hostName, processName) {
    this._withHostAndProcess(hostName, processName, function (host, process) {
      this.trigger('page', new ProcessPage({
        model: process,
        runningPage: ProcessSnapshotsPage
      }))

      App.view.setActiveNav('/host/' + hostName + '/process/' + processName + '/snapshots')
    }.bind(this))
  },
