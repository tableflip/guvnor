...
const launchdListProcessCertificateFingerprints = () => {
  ...
  return allPlists()
  .then(plists => plists.filter(plist => plist && plist.Environmentconstiables && plist.Environmentconstiables[`${config.DAEMON_ENV_NAME}_CERT`]))
  .then(plists => Promise.all(
    plists.map(
      plist => pem.getFingerprint(plist.Environmentconstiables[`${config.DAEMON_ENV_NAME}_CERT`])
      .then(fingerprint => {
