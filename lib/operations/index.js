      ...
      Joi.validate(args, schema[key].args, function (error, values) {
        if (error) {
          if (!context || typeof context.log !== 'function') {
            console.error(`Please pass a context object as the first argument to ${key}`)
          } else {
            context.log(`Failed to validate ${key} input: ${stringify(values, null, 2)}`)
            context.log(error)
          }
        ...
        promise.then(result => {
          Joi.validate(result, schema[key].then, function (error, values) {
            if (error) {
              context.log(`Failed to validate ${key} output: ${stringify(values, null, 2)}`)
              context.log(error)
