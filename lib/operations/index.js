...
const api = {
  ...
  fetchLogs: require(`./${processManager}/fetch-logs`),
  ...
  findApp: require('./common/find-app'),
  ...
  sendEvent: require('./common/send-event'),
  sendSignal: require('./common/send-signal')
}
      ...
      Joi.validate(args, schema[key].args, function (error, values) {
        if (error) {
          if (!context || typeof context.log !== 'function') {
            ...
            context.log([ERROR, CONTEXT], `Failed to validate ${key} input: ${stringify(values, null, 2)} at ${stack} during request ${context.id}`)
        ...
        promise.then(result => {
          Joi.validate(result, schema[key].then, function (error, values) {
            if (error) {
              context.log([ERROR, CONTEXT], `Failed to validate ${key} output: ${stringify(values, null, 2)} at ${stack} during request ${context.id}`)
              context.log([ERROR, CONTEXT], error)
