...
const app = Joi.object({
  ...
  group: Joi.string().required(),
  valid: Joi.boolean(),
  version: Joi.string().when('valid', {is: true, then: Joi.required()}),
  url: Joi.string().when('valid', {is: true, then: Joi.required()}),
  ref: appRef.when('valid', {is: true, then: Joi.required()}),
