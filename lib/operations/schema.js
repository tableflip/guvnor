...
module.exports = {
  ...
  listProcessDetails: {
    ...
    output: Joi.object({
      0: Joi.object().type(Error).empty(null).label('error'),
      1: Joi.array().items(procDetails).label('detailed process list')
    })
  },
  removeProcess: {
    args: Joi.object({
  ...
  fetchHeapSnapshot: {
    args: Joi.object({
      ...
      3: Joi.func().label('on details callback').required(),
      4: Joi.func().label('on data callback').required(),
      5: Joi.func().label('callback').required()
    }),
    output: Joi.object({
      0: Joi.object().type(Error).empty(null)
    })
  ...
  removeHeapSnapshot: {
    ...
    output: Joi.object({
      0: Joi.object().type(Error).empty(null),
      1: heapSnapshot.empty(null).label('snapshot list')
    })
  },
  takeHeapSnapshot: {
    args: Joi.object({
  ...
  fetchProcessLogs: {
    args: Joi.object({
      0: context.required(),
      1: Joi.string().label('script name').required(),
      2: Joi.func().label('on details callback').required(),
      3: Joi.func().label('on data callback').required(),
  ...
  setNumWorkers: {
    args: Joi.object({
      0: context.required(),
      1: Joi.string().label('script name').required(),
      2: Joi.number().integer().min(1).max(os.cpus().length).label('number of workers').required(),
      3: Joi.func().label('callback').required()
    }),
    output: Joi.object({
      0: Joi.object().type(Error).empty(null)
