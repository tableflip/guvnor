...
const createUnit = (context, options) => {
  ...
  console.info('-----> args', JSON.stringify(args, null, 2))
...
const createEnv = (context, options) => {
  ...
  env[`${config.DAEMON_ENV_NAME}_WORKERS`] = options.workers
...
const systemdCreateProcess = (context, options, callback) => {
  options = JSON.parse(JSON.stringify(options))

  options.unitFile = path.join(config.UNIT_FILE_LOCATIONS, `${config.DAEMON_NAME}.${options.name}.service`)
  options.envFile = path.join(config.UNIT_FILE_LOCATIONS, `${config.DAEMON_NAME}.${options.name}.env`)
  options.keyFile = path.join(config.UNIT_FILE_LOCATIONS, `${config.DAEMON_NAME}.${options.name}.key`)
  options.certFile = path.join(config.UNIT_FILE_LOCATIONS, `${config.DAEMON_NAME}.${options.name}.cert`)
  options.caFile = path.join(config.UNIT_FILE_LOCATIONS, `${config.DAEMON_NAME}.${options.name}.ca`)
