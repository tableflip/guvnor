...
module.exports = function findProcessStatus (context, proc, callback) {
  ...
  connectToProcess(context, proc, (error, remote, disconnect) => {
    if (error) {
    ...
    remote.reportStatus((error, results) => {
      disconnect()
      ...
      const uids = unique([results.master.uid].concat(results.workers.map(function (worker) {
        return worker.uid
      })))

      const gids = unique([results.master.gid].concat(results.workers.map(function (worker) {
        return worker.gid
      })))
