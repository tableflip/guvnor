...
const updateApp = (context, name, outputStream, callback) => {
  ...
  .then(app => {
    ...
    .then(() => runCommand(config.GIT_PATH, ['config', 'core.bare', 'true'], repo, outputStream, `Changing repository ${repo} to bare failed`))
    .then(() => runCommand(config.GIT_PATH, ['remote', 'update', '--prune'], repo, outputStream, `Updating remote history for ${repo} failed`))
    .then(() => runCommand(config.GIT_PATH, ['config', 'core.bare', 'false'], repo, outputStream, `Changing repository ${repo} to non-bare failed`))
    .then(() => resetApp(context, app.path, outputStream))
    .then(() => runCommand(config.GIT_PATH, ['checkout', app.ref.name], app.path, outputStream, `Checking out ref ${app.ref.name} in ${app.path} failed`))
