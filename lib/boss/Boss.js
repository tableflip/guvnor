...
Boss.prototype.stopProcess = function(pid, options, callback) {
  this._logger.debug('stopping', pid)

  var processInfo = this._processes[pid]

  if(!processInfo) {
    this._logger.debug('no process', pid)
    return callback(new Error('There is no process with the pid ' + pid))
  }

  this._logger.debug('killing', pid)
