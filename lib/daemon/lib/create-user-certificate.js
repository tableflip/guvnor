...
module.exports = (context, ca, user, callback) => {
  ...
  const certificateOpts = {
    ...
    organization: config.DAEMON_NAME_SENTENCE_CASE,
    organizationUnit: 'user',
    serviceKey: ca.key,
    serviceCertificate: ca.certificate,
    serial: Date.now(),
    days: 4096,
    extFile: path.join(__dirname, 'cert-ext.txt')
  }
  ...
  .then(certs => Promise.all([
    certs,
    fs.chown(keyFile, user.uid, user.gid),
    fs.chown(certificateFile, user.uid, user.gid)
  ]))
  .then(([certs]) => {
