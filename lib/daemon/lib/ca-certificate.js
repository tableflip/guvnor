...
module.exports = (context) => {
  if (!promise) {
    promise = readOrCreateCertificate(context, CA_KEY_PATH, CA_CERT_PATH, {
      days: CA_CERT_DAYS,
      commonName: `${config.DAEMON_NAME_SENTENCE_CASE} CA Certificate`,
      extFile: path.join(__dirname, 'cert-ext-ca.txt')
    }, (result) => {
      return {
