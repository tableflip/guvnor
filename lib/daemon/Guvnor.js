...
Guvnor.prototype.kill = function (userDetails, callback) {
  ...
  if (this._config.guvnor.autoresume) {
    this.dumpProcesses(this._kill.bind(this, callback))
  } else {
    this._kill(callback)
  }
}
...
Guvnor.prototype.switchApplicationRef = function (userDetails, name, ref, onOut, onErr, callback) {
  this._appService.switchRef(name, ref, onOut, onErr, function(error, appInfo, previousRef, newRef) {
    callback(error, appInfo, previousRef, newRef)

    this.emit('app:refs:switched', error, appInfo, previousRef, newRef)
  }.bind(this))
}
...
Guvnor.prototype.updateApplicationRefs = function (userDetails, name, onOut, onError, callback) {
  this._appService.updateRefs(name, onOut, onError, function(error, appInfo, refs) {
...
Guvnor.prototype.generateRemoteRpcCertificates = function (userDetails, days, callback) {
  this._pem.createCertificate({
    ...
    var configPath = this._config.guvnor.confdir + '/guvnorrc'
