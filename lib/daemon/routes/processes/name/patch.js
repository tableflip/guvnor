  ...
  server.route({
    ...
    handler: function updateProcessHandler (request, reply) {
      ...
      if (request.payload.status === 'start') {
        ...
        request.server.methods.stopProcess(context, request.params.name, (error) => {
          reply(error)

          if (!error) {
            request.server.methods.findProcess(context, request.params.name, function (error, proc) {
              if (proc) {
