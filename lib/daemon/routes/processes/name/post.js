...
const CONTEXT = 'daemon:routes:processes:name:post'
...
const createProcess = (request, reply) => {
  ...
  // did we request an app
  return request.server.methods.listApps(context(request))
  .then(apps => apps.find(app => app.name === request.params.name))
  .then(app => {
    if (app) {
      request.log([INFO, CONTEXT], `${request.params.name} resolved to an app`)
      // start an app instead of a script
      options.script = app.path
    }
