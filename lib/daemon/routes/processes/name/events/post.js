...
const receiveEvent = (request, name, event, args) => {
  request.log([DEBUG, CONTEXT], `${name} emitted ${event}`)
...
const createProcessEvent = (request) => {
  console.info('incoming event from', request.auth.credentials, `${request.params.name} emitted ${request.payload.event}`)
  if (request.auth.credentials.scope.indexOf('process') !== -1) {
    ...
    request.log([DEBUG, CONTEXT], `Sending ${request.payload.event} to ${request.params.name} with args ${request.payload.args}`)

    return request.server.methods.sendEvent(context(request), request.params.name, request.payload.event, request.payload.args)
  }
