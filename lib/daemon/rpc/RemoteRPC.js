...
RemoteRPC.prototype.connectToProcess = function (userDetails, processId, callback) {
  ...
  this._guvnor.findProcessInfoById(userDetails, processId, function (error, processInfo) {
    ...
    this._connectToRpc(processInfo.socket, userDetails, callback)
  }.bind(this))
}

RemoteRPC.prototype._connectToRpc = function (socket, userDetails, callback) {
  var child
...
RemoteRPC.prototype._startProcess = function (userDetails, script, options, callback) {
  ...
  if (options.user && options.user !== userDetails.name) {
    // only root is allowed to run processes for other users
    socket = 'admin.socket'
  }

  this._connectToRpc(this._fileSystem.getRunDir() + '/' + socket, userDetails, function (error, guvnor) {
    if (error) {
      return callback(error)
    }
