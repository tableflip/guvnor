...
ProcessService.prototype._handleProcessExit = function (processInfo, prefix, code, signal) {
  if (signal) {
    ...
    processInfo.logger.error('Child process %s (%s) exited with code %d and signal %s', processInfo.name, processInfo.process.pid, code, signal)
  } else {
    this._logger.debug('Child process %s (%s) exited with code %d', processInfo.name, processInfo.process.pid, code)
    processInfo.logger.error('Child process %s (%s) exited with code %d', processInfo.name, processInfo.process.pid, code)
  }
  ...
  if (processInfo.status !== 'stopping' && (this._exitWasUnclean(code) || processInfo.status === 'restarting')) {
...
ProcessService.prototype._handleProcessError = function (processInfo, prefix, error) {
  this._logger.error('Child process', processInfo.name, 'emitted error event', error.stack ? error.stack : error.message ? error.message : error)
