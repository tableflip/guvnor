...
ProcessService.prototype.startProcess = function(script, options, callback) {
  ...
  if(arguments.length == 2 && script instanceof ProcessInfo && typeof options == 'function') {
    ...
    return this._startProcessInfo(processInfo, callback)
  }

  processInfo = this._processInfoStore.find('name', options.name) || this._processInfoStore.find('name', script)

  if(processInfo) {
    if(['uninitialised', 'starting', 'started', 'running', 'restarting', 'stopping'].indexOf(processInfo.status) != -1) {
      return callback(new Error(processInfo.name + ' is already running'))
    }

    processInfo.setOptions(options, function(error) {
      if(error) {
        return callback(error)
      }

      this._startProcessInfo(processInfo, callback)
    }.bind(this))
...
ProcessService.prototype._startProcessInfo = function(processInfo, callback) {
  this._startProcess(processInfo, function(error) {
    if(error) {
      // happens when we fail to reserve a debug port
      processInfo.status = 'failed'
      return this.emit('process:failed', processInfo, error)
    }
...
ProcessService.prototype._startProcess = function(processInfo, callback) {
  this._addDebugPort(processInfo, function(error) {
...
ProcessService.prototype._setupProcessCallbacks = function(processInfo, prefix) {
  ...
  processInfo.process.on('exit', function(code, signal) {
    ...
    if(code === null || code !== 0 || processInfo.status == 'restarting') {
