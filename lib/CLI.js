    ...
    if(error.message == 'group id does not exist') {
      ...
      this._prompt.get([{
        ...
        if(command) {
          ...
          this._execSync.run(command + ' ' + this._config.boss.group)
        ...
        if(command) {
          ...
          // find a gid
          this._execSync.exec(command + ' . -list /Groups PrimaryGroupID').stdout.trim().split('\n').forEach(function(line) {
          ...
          this._execSync.run(command + ' . create /Groups/' + this._config.boss.group)
          this._execSync.run(command + ' . create /Groups/' + this._config.boss.group + ' name ' + this._config.boss.group)
          this._execSync.run(command + ' . create /Groups/' + this._config.boss.group + ' passwd "*"')
          this._execSync.run(command + ' . create /Groups/' + this._config.boss.group + ' gid ' + gid)

          this._fs.appendFileSync('/etc/group', this._config.boss.group + ':*:' + gid + ':\n')

          return callback()
        }

        callback(new Error('I don\'t know how to create groups, sorry!'))
      }.bind(this))
    } else {
      callback(error)
    }
