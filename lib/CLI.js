...
CLI.prototype.stop = function(name, options) {
	this._boss.invoke(function() {
		this._boss.stopProcess(name, options, function(error) {
			if(error) {
				console.error(error);
			}
...
CLI.prototype.cluster = function(name, options) {
	console.info("name", name, "options", options);

	this._boss.invoke(function() {
		if(options.workers) {
			this._boss.setClusterWorkers(name, options.workers, function(error) {
				if(error) {
					console.error(error);
				}
