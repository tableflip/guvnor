...
CLI.prototype.afterPropertiesSet = function() {
  ...
  commander
    .command('stop <pid> [pids...]')
      .description('Stop a process')
      .option('-v, --verbose', 'Prints detailed internal logging output')
      .action(this.stop.bind(this))

  commander
    .command('restart <pid> [pids...]')
...
CLI.prototype.stop = function(pid, pids, options) {
  pids = [pid].concat(pids || [])
  ...
  this._do(options, function(boss) {
    async.eachSeries(pids, function(pid, callback) {
      this._withRemoteProcess(boss, pid, function(error, remoteProcess) {
        ...
        this._logger.debug('Killing remote process ' + pid)
        remoteProcess.kill()
...
CLI.prototype.restart = function(pid, pids, options) {
  pids = [pid].concat(pids || [])
  ...
  this._do(options, function(boss) {
    async.eachSeries(pids, function(pid, callback) {
      this._withRemoteProcess(boss, pid, function(error, remoteProcess) {
        ...
        this._logger.debug('Restarting remote process ' + pid)
...
CLI.prototype._logBossMessages = function(boss) {
  boss.on('*', function() {
    ...
    Array.prototype.slice.call(arguments).forEach(function(arg) {
      ...
      args.push(str.length > 20 ? str.substr(0, 20) + '...' : str)
    })
