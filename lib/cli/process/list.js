...
module.exports = function start (user, api, yargs) {
  var argv = yargs
    ...
    .describe('detail', 'Prints detailed process information')
    .alias('d', 'detail')
    .boolean('d')
  ...
  api.process.list(function (error, processes) {
    ...
    if (argv.detail) {
      ...
      table.addHeader(['PID', 'User', 'Group', 'Name', 'Uptime', 'CPU', 'RSS', 'Heap size', 'Heap used', 'Status', 'Type'])
      ...
      var addProcessToTable = function (proc, type) {
        ...
        table.addRow([pid, user, group, name, uptime, cpu, rss, heapTotal, heapUsed, status, type])
      }

      processes.forEach(function (proc) {
        addProcessToTable(proc, proc.workers ? 'Manager' : 'Process')

        if (Array.isArray(proc.workers)) {
