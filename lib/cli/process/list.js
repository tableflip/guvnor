...
module.exports = (user, api, stdout, stderr, yargs) => {
  const argv = yargs
  ...
  .then(processes => {
    ...
    if (argv.json) {
      return stdout.write(stringify(processes, null, 2))
    }
    ...
    if (stdout.isTTY) {
      ...
      return table.print(stdout.write)
    }

    stdout.write(processes.map((proc) => {
      return proc.name
    }).join('\n'))
  })
}
