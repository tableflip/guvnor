...
module.exports = (process) => {
  ...
  const buffer = new OutputBuffer((line) => {
    ...
    if (line) {
      promises.push(run(args.concat(line), process.stdout, process.stderr))
    }
  })
  ...
  process.stdin.on('readable', () => {
    ...
    if (chunk === null) {
      if (buffer.size() === 0 && !readData) {
        // not being piped to
        promises.push(run(args, process.stdout, process.stderr))
...
const run = (args, stdout, stderr) => {
  ...
  .then(user => {
    ...
    return loadApi(user.keyBundle).then(api => {
      return {
        user: user,
        api: api
      }
  ...
  .then(results => {
    const bind = (fn) => {
      return fn.bind(null, results.user, results.api, stdout, stderr)
    }
