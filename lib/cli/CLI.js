...
var CLI = function() {
  ...
  this._remoteProcessFactory = Autowire
}
...
CLI.prototype.stop = function(pid, options) {
  ...
  this._logger.debug('Stopping pid', pid)

  var socket = this._fileSystem.findProcessDirectory() + '/' + pid

  this._logger.debug('Connecting to remote process at', socket)

  var remote = this._remoteProcessFactory.create(socket)
  remote.connect(function(error) {
