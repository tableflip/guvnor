...
CLI.prototype.afterPropertiesSet = function() {
  ...
  async.series([
    this._checkBossUser.bind(this),
    this._checkBossGroup.bind(this)
  ], function(error) {
...
CLI.prototype._setUpCommander = function() {
  ...
  this._commander
    .command('web')
    .description('Starts boss-web as a managed process')
    .option('-u, --user <user>', 'The user to start a process as')
    .option('-g, --group <group>', 'The group to start a process as')
    .option('-d, --debug', 'Pause the process at the start of execution and wait for a debugger to be attached')
    .option('-v, --verbose', 'Prints detailed internal logging output')
    .action(this._processes.startBossWeb.bind(this._processes))
...
CLI.prototype._checkBossUser = function(callback) {
  ...
  try {
    bossGroup = this._posix.getgrnam(this._config.boss.group)
  } catch(e) {
    if(e.message != 'group id does not exist') {
      return callback(e)
    }

    this._logger.error('Boss has been configured to run as %s:%s but the %s group does not exist.', this._config.boss.user, this._config.boss.group, this._config.boss.group)
    this._logger.error('')
    this._logger.error('To fix this run boss as %s and it will attempt to create the %s group.', this._config.boss.user, this._config.boss.group)
    this._logger.error('')

    return callback(new Error(this._config.boss.group + ' group does not exist'))
  }

  if(bossGroup.members.indexOf(this._user.name) == -1) {
    this._logger.error('Boss has been configured to run as %s:%s but user %s is not in the group %s', this._config.boss.user, this._config.boss.group , this._user.name, this._config.boss.group)
    this._logger.error('')

    if(this._os.platform() == 'linux') {
      this._logger.error('To fix this run:')
      this._logger.error('')
      this._logger.error('$ sudo usermod -G %s -a %s', this._config.boss.group, this._user.name)
      this._logger.error('')
    } else if(this._os.platform() == 'darwin') {
      this._logger.error('To fix this run:')
      this._logger.error('')
      this._logger.error('$ sudo dscl . append /Groups/%s GroupMembership %s', this._config.boss.group, this._user.name)
      this._logger.error('')
    } else {
      this._logger.error('To fix this add % to the % group.', this._user.name, this._config.boss.group)
      this._logger.error('')
    }
  ...
  this._running(function(running) {
    callback(running ? undefined : new Error('The daemon is not running. Please run this command as ' + this._config.boss.user + ' to start the daemon.'))
  }.bind(this))
}

CLI.prototype._checkBossGroup = function(callback) {
  try {
    this._posix.getgrnam(this._config.boss.group)

    return callback()
  } catch(error) {
    if(error.message == 'group id does not exist') {

      this._logger.warn('Boss has been configured to start with the group \'%s\' but that group does not exist', this._config.boss.group)
      this._prompt.start()
      this._prompt.get([{
        name: 'create group \'' + this._config.boss.group + '\' [Y/n]',
        default: 'Y'
      }], function (error) {
        if(error) return callback(error)

        this._logger.debug('Creating group', this._config.boss.group)

        // ugh http://www.greenend.org.uk/rjk/tech/useradd.html

        // linux
        var command = this._execSync.exec('which groupadd').stdout.trim()

        if(command) {
          command = command + ' ' + this._config.boss.group
          this._logger.debug(command)

          var code = this._execSync.run(command)

          if(code !== 0) {
            error = new Error('Automatically creating group ' + this._config.boss.group + ' failed, please create it manually')
          }
        ...
        // mac os x
        command = this._execSync.exec('which dscl').stdout.trim()
          ...
          async.series([
            function(callback) {
              this._child_process.exec(command + ' . -list /Groups PrimaryGroupID', function(error, stdout) {
                ...
                this._logger.debug(this._config.boss.group + ' will have gid ' + gid)

                callback()
              }.bind(this))
            }.bind(this),
            function(callback) {
              this._child_process.exec(command + ' . create /Groups/' + this._config.boss.group, callback)
            }.bind(this),
            function(callback) {
              this._child_process.exec(command + ' . create /Groups/' + this._config.boss.group + ' name ' + this._config.boss.group, callback)
            }.bind(this),
            function(callback) {
              this._child_process.exec(command + ' . create /Groups/' + this._config.boss.group + ' passwd "*"', callback)
            }.bind(this),
            function(callback) {
              this._child_process.exec(command + ' . create /Groups/' + this._config.boss.group + ' gid ' + gid, callback)
            }.bind(this),
            function(callback) {
              this._fs.appendFile('/etc/group', this._config.boss.group + ':*:' + gid + ':\n', callback)
            }.bind(this)
          ], callback)
        ...
        callback(new Error('Automatically creating group ' + this._config.boss.group + ' failed, please create it manually'))
      }.bind(this))
    } else {
      callback(error)
    }
