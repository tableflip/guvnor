    ...
    boss.listProcesses(function(error, processes) {
      ...
      var addProcessToTable = function(proc, type) {
        ...
        var pid = proc.pid ? proc.pid : '?',
          ...
          rss = isNaN(proc.residentSize) ? '?' : this._formatMemory(proc.residentSize, true),
          heapTotal = isNaN(proc.heapTotal) ? '?' : this._formatMemory(proc.heapTotal, true),
          heapUsed = isNaN(proc.heapUsed) ? '?' : this._formatMemory(proc.heapUsed, true),
