...
const convertError = (method, error, response) => {
...
const handleResponse = (method, callback, error, response) => {
  if (error) {
    error = convertError(method, error, response)
...
const handleStreamingResponse = (method, output, callback, error, response) => {
  if (error) {
    error = convertError(method, error, response)
    ...
    if (returnArgs.length) {
      ...
      if (serverError) {
        ...
        if (serverError.isBoom) {
          message.push(serverError.output.payload.message)
        ...
        if (serverError.syscall) {
          message.push(`syscall: ${serverError.syscall}`)
        }

        if (serverError.path) {
          message.push(`path: ${serverError.path}`)
        }

        if (serverError.spawnargs && Array.isArray(serverError.spawnargs)) {
          message.push(`args: ${serverError.spawnargs.join(' ')}`)
        }

        if (serverError.code) {
          message.push(`code: ${serverError.code}`)
        }

        if (response.headers['guvnor-request-id']) {
          message.push(`request: ${response.headers['guvnor-request-id']}`)
        }
...
const handleStreamingDataResponse = (method, data, callback, error, response) => {
  if (error) {
    error = convertError(method, error, response)
...
module.exports = (keyBundle) => {
  ...
  const wreck = Wreck.defaults({
    ...
    agent: new https.Agent({
      ...
      cert: keyBundle.certificate || keyBundle.cert,
      key: keyBundle.key,
      ca: keyBundle.ca
    })
  })
  ...
  return {
    ...
    request: (opts, callback) => {
      opts = opts || {}
      opts.payload = opts.payload ? JSON.stringify(opts.payload) : undefined
      ...
      if (opts.output) {
        responseHandler = handleStreamingResponse.bind(null, opts.method, opts.output, callback)
      } else if (opts.data) {
        responseHandler = handleStreamingDataResponse.bind(null, opts.method, opts.data, callback)
      } else {
        responseHandler = handleResponse.bind(null, opts.method, callback)
      }

      wreck.request(opts.method, opts.path, {
